...
declarations {
	...
	§ the clause the two rails disagree. A call naming the Namespace reads the
	§ written Method, and the derive answers every bounded one. The derive is
	§ built from the Choice and consults no Namespace. So writing the Method
	§ undeclared is refused, under `undeclared-conformance`. A Choice with a
	§ payload anywhere derives nothing, and conforms by writing the Method.
