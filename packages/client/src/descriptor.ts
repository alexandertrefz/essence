...
// NOTE: The COMPILE-TIME half of the boundary — the only half that knows what a
// `common.Type` is. Everything here reads the Compiler's own vocabulary once and
// writes down what it found: a Descriptor, which is plain JSON, which the
// interpreter in `marshal-runtime.ts` marshals by without ever meeting a Type.
//
...
// NOTE: What an `Optional` holds, or `null` where the Type is not one. An
...
//
// NOTE: Exported, with `admitsAbsence` below, because the generated declarations
// refuse exactly what the interpreter refuses — a second spelling of the
...
export function optionalItemOf(type: common.Type): common.Type | null {
	switch (type.type) {
		case "Case":
			return isOptionalCase(type, "Value")
...
// NOTE: Whether `undefined` is a value of this Type — which is exactly whether an
// `Optional`'s `#Empty` is reachable in it. What it decides is whether an
...
export function admitsAbsence(type: common.Type): boolean {
	switch (type.type) {
		case "Case":
			return isOptionalCase(type, "Empty")
		case "UnionType":
