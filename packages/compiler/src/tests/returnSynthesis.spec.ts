...
// NOTE: Regression tests — a body promising Nothing is allowed to fall off its
// end, and the emitted JavaScript Function then answered `undefined`. That is
// not an Essence value: it carries no hidden Type key, so the failure surfaced
// somewhere else entirely, in whatever read that key next — printing the
// result, holding it in a Record, or matching on it all died with a `TypeError`
// naming the runtime's internals, out of a Program that compiled with zero
// Diagnostics.
describe("Return Synthesis", () => {
	describe("a Function body that falls off its end", () => {
		it("answers the Nothing its Signature promised", async () => {
			expect(
		...
		it("answers one a Match can narrow", async () => {
			expect(
				await run(`implementation {
					...
					constant value: Integer | Nothing = noop()
	...
	// the Match's value.
	it("answers Nothing for a Handler that writes no Return", async () => {
		expect(
			await run(`implementation {
				constant value: Integer | Nothing = 1

				__print(match value -> Nothing {
					case Integer {
					}
					case Nothing {
					}
	...
	// a body that already ends in `<- nothing` must not grow a second,
	// unreachable one behind it.
	it("leaves a body that returns explicitly alone", () => {
		let generated = generate(`implementation {
			function explicit () -> Nothing {
				<- nothing
			}
