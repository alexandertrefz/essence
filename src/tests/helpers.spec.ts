...
describe("Helpers", () => {
	...
	describe("matchesType", () => {
		...
		const booleanPrimitive: PrimitiveType = { type: "Primitive", primitive: "Boolean" }
		const integerPrimitive: PrimitiveType = { type: "Primitive", primitive: "Integer" }
		const fractionPrimitive: PrimitiveType = { type: "Primitive", primitive: "Fraction" }
		const stringPrimitive: PrimitiveType = { type: "Primitive", primitive: "String" }
		...
		const recordType: Type = {
			type: "Type",
			name: "TestType",
			definition: { type: "Record", members: { test: { type: "Primitive", primitive: "String" } } },
			methods: {},
		}
		...
		const alternativeRecordType: Type = {
			type: "Type",
			name: "TestType",
			definition: { type: "Record", members: { test: { type: "Primitive", primitive: "Integer" } } },
			methods: {},
		}
		...
		const unknownList: ListType = { type: "List", itemType: { type: "Unknown" } }
		const stringList: ListType = { type: "List", itemType: { type: "Primitive", primitive: "String" } }
		const integerList: ListType = { type: "List", itemType: { type: "Primitive", primitive: "Integer" } }
		...
		const singleArgumentWithNameFunctionType: FunctionType = {
			type: "Function",
			parameterTypes: [{ name: "test", type: { type: "Primitive", primitive: "String" } }],
			returnType: { type: "Primitive", primitive: "String" },
		}
		...
		const noArgumentOverloadedMethodType: OverloadedMethodType = {
			...
			overloads: [{ parameterTypes: [], returnType: { type: "Primitive", primitive: "String" } }],
		}
		...
		const singleArgumentWithNameOverloadedMethodType: OverloadedMethodType = {
			...
			overloads: [
				...
				{
					parameterTypes: [{ name: "test2", type: { type: "Primitive", primitive: "String" } }],
					returnType: { type: "Primitive", primitive: "String" },
				},
			],
		}
		...
		const alternativeSingleArgumentWithNameOverloadedMethodType: OverloadedMethodType = {
			...
			overloads: [
				...
				{
					parameterTypes: [{ name: "test3", type: { type: "Primitive", primitive: "String" } }],
					returnType: { type: "Primitive", primitive: "String" },
				},
			],
		}
		...
		it("should match matching FunctionTypes", () => {
			expect(matchesType(noArgumentFunctionType, noArgumentFunctionType)).toBe(true)
			expect(matchesType(singleArgumentFunctionType, singleArgumentFunctionType)).toBe(true)
			expect(matchesType(singleArgumentWithNameFunctionType, singleArgumentWithNameFunctionType)).toBe(true)
			expect(
				matchesType(
		...
		it("should not match mismatched FunctionTypes", () => {
			expect(matchesType(singleArgumentFunctionType, noArgumentFunctionType)).toBe(false)
			expect(matchesType(singleArgumentFunctionType, singleArgumentWithNameFunctionType)).toBe(false)
			expect(matchesType(noArgumentFunctionType, noArgumentWithDifferentReturnTypeFunctionType)).toBe(false)
			expect(matchesType(noArgumentFunctionType, singleArgumentFunctionType)).toBe(false)
			expect(matchesType(noArgumentFunctionType, singleArgumentWithNameFunctionType)).toBe(false)
			expect(matchesType(noArgumentWithDifferentReturnTypeFunctionType, noArgumentFunctionType)).toBe(false)
		})

		it("should match matching SimpleMethodTypes", () => {
			expect(matchesType(noArgumentSimpleMethodType, noArgumentSimpleMethodType)).toBe(true)
			expect(matchesType(singleArgumentSimpleMethodType, singleArgumentSimpleMethodType)).toBe(true)
			expect(matchesType(singleArgumentWithNameSimpleMethodType, singleArgumentWithNameSimpleMethodType)).toBe(
				true,
			)
			expect(
				matchesType(
		...
		it("should not match mismatched SimpleMethodTypes", () => {
			expect(matchesType(singleArgumentSimpleMethodType, noArgumentSimpleMethodType)).toBe(false)
			expect(matchesType(singleArgumentSimpleMethodType, singleArgumentWithNameSimpleMethodType)).toBe(false)
			expect(matchesType(noArgumentSimpleMethodType, noArgumentWithDifferentReturnTypeSimpleMethodType)).toBe(
				false,
			)
			expect(matchesType(noArgumentSimpleMethodType, singleArgumentSimpleMethodType)).toBe(false)
			expect(matchesType(noArgumentSimpleMethodType, singleArgumentWithNameSimpleMethodType)).toBe(false)
			expect(matchesType(noArgumentWithDifferentReturnTypeSimpleMethodType, noArgumentSimpleMethodType)).toBe(
				false,
			)
		})

		it("should match matching StaticMethodTypes", () => {
			expect(matchesType(noArgumentStaticMethodType, noArgumentStaticMethodType)).toBe(true)
			expect(matchesType(singleArgumentStaticMethodType, singleArgumentStaticMethodType)).toBe(true)
			expect(matchesType(singleArgumentWithNameStaticMethodType, singleArgumentWithNameStaticMethodType)).toBe(
				true,
			)
			expect(
				matchesType(
		...
		it("should not match mismatched StaticMethodTypes", () => {
			expect(matchesType(singleArgumentStaticMethodType, noArgumentStaticMethodType)).toBe(false)
			expect(matchesType(singleArgumentStaticMethodType, singleArgumentWithNameStaticMethodType)).toBe(false)
			expect(matchesType(noArgumentStaticMethodType, noArgumentWithDifferentReturnTypeStaticMethodType)).toBe(
				false,
			)
			expect(matchesType(noArgumentStaticMethodType, singleArgumentStaticMethodType)).toBe(false)
			expect(matchesType(noArgumentStaticMethodType, singleArgumentWithNameStaticMethodType)).toBe(false)
			expect(matchesType(noArgumentWithDifferentReturnTypeStaticMethodType, noArgumentStaticMethodType)).toBe(
				false,
			)
		})

		it("should match matching OverloadedMethodTypes", () => {
			expect(matchesType(noArgumentOverloadedMethodType, noArgumentOverloadedMethodType)).toBe(true)
			expect(matchesType(singleArgumentOverloadedMethodType, singleArgumentOverloadedMethodType)).toBe(true)
			expect(
				matchesType(singleArgumentWithNameOverloadedMethodType, singleArgumentWithNameOverloadedMethodType),
			).toBe(true)
			expect(
				matchesType(
		...
		it("should not match mismatched OverloadedMethodTypes", () => {
			expect(matchesType(singleArgumentOverloadedMethodType, noArgumentOverloadedMethodType)).toBe(false)
			expect(matchesType(singleArgumentOverloadedMethodType, singleArgumentWithNameOverloadedMethodType)).toBe(
				false,
			)
			expect(
				matchesType(noArgumentOverloadedMethodType, noArgumentWithDifferentReturnTypeOverloadedMethodType),
			).toBe(false)
			expect(matchesType(noArgumentOverloadedMethodType, singleArgumentOverloadedMethodType)).toBe(false)
			expect(matchesType(noArgumentOverloadedMethodType, singleArgumentWithNameOverloadedMethodType)).toBe(false)
			expect(
				matchesType(noArgumentWithDifferentReturnTypeOverloadedMethodType, noArgumentOverloadedMethodType),
			).toBe(false)
			expect(
				matchesType(
		...
		it("should match matching OverloadedStaticMethodTypes", () => {
			expect(matchesType(noArgumentOverloadedStaticMethodType, noArgumentOverloadedStaticMethodType)).toBe(true)
			expect(
				matchesType(singleArgumentOverloadedStaticMethodType, singleArgumentOverloadedStaticMethodType),
			).toBe(true)
			expect(
				matchesType(
		...
		it("should not match mismatched OverloadedStaticMethodTypes", () => {
			expect(matchesType(singleArgumentOverloadedStaticMethodType, noArgumentOverloadedStaticMethodType)).toBe(
				false,
			)
			expect(
				matchesType(singleArgumentOverloadedStaticMethodType, singleArgumentWithNameOverloadedStaticMethodType),
			).toBe(false)
			expect(
				matchesType(
			...
			expect(matchesType(noArgumentOverloadedStaticMethodType, singleArgumentOverloadedStaticMethodType)).toBe(
				false,
			)
			expect(
				matchesType(noArgumentOverloadedStaticMethodType, singleArgumentWithNameOverloadedStaticMethodType),
			).toBe(false)
			expect(
				matchesType(
					noArgumentWithDifferentReturnTypeOverloadedStaticMethodType,
		...
		it("should not match mismatched Types", () => {
			expect(matchesType(recordType, noArgumentOverloadedStaticMethodType)).toBe(false)
			expect(matchesType(builtInType, singleArgumentWithNameOverloadedStaticMethodType)).toBe(false)
			expect(matchesType(builtInType, fractionPrimitive)).toBe(false)
			expect(matchesType(fractionPrimitive, builtInType)).toBe(false)
			expect(matchesType(noArgumentOverloadedStaticMethodType, singleArgumentFunctionType)).toBe(false)
			expect(matchesType(noArgumentOverloadedMethodType, singleArgumentOverloadedStaticMethodType)).toBe(false)
			expect(matchesType(noArgumentOverloadedStaticMethodType, fractionPrimitive)).toBe(false)
			expect(matchesType(integerList, noArgumentOverloadedStaticMethodType)).toBe(false)
		})
	})
})
