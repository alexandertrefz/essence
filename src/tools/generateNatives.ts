...
// NOTE: A Namespace's contract: the `*Natives` type listing every native entry
// (Properties first, in source order, then Methods, each Overload as its own
// mangled `__overload$N` key), and — only when the Namespace has any
// Essence-bodied member — a `*EssenceImplemented` type with one optional-`never`
// key per bodied entry, so a stray runtime export for a bodied Method is a
// type error.
function renderNamespace(
	...
	if (bodiedKeys.length > 0) {
		...
		sections.push(
			`export type ${namespace.name}EssenceImplemented = {\n${absence}\n}`,
		)
	}
