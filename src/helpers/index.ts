...
export function matchesType(lhs: common.Type, rhs: common.Type): boolean {
	...
	if (lhs.type === "Type" && rhs.type === "Type") {
		if (lhs.definition.type === "BuiltIn" && rhs.definition.type === "BuiltIn") {
			return true
		} else if (lhs.definition.type !== "BuiltIn" && rhs.definition.type !== "BuiltIn") {
	...
	if (lhs.type === "Type" && (rhs.type === "Primitive" || rhs.type === "Record")) {
	...
	if ((lhs.type === "Primitive" || rhs.type === "Record") && rhs.type === "Type") {
	...
	if (lhs.type === "OverloadedMethod" && rhs.type === "OverloadedMethod") {
		...
		for (let i = 0; i < lhs.overloads.length; i++) {
			...
			if (lhsOverload.parameterTypes.length !== rhsOverload.parameterTypes.length) {
				return false
			}

			for (let j = 0; j < lhsOverload.parameterTypes.length; j++) {
				if (
					lhsOverload.parameterTypes[j].name !== rhsOverload.parameterTypes[j].name ||
					!matchesType(lhsOverload.parameterTypes[j].type, rhsOverload.parameterTypes[j].type)
				) {
					return false
				}
			}
	...
	if (lhs.type === "OverloadedStaticMethod" && rhs.type === "OverloadedStaticMethod") {
		if (lhs.overloads.length !== rhs.overloads.length) {
			return false
		}
